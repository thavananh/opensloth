...
WEIGHT_SYNC_INTERVAL = 64  # Steps between weight synchronization
WEIGHT_FILE_WAIT_TIMEOUT = 1800  # 30 minutes in seconds
LOCK_FILE_WAIT_TIMEOUT = 60  # 60 seconds


def setup_model_and_training(gpu_id: int, all_gpu_ids: List[int], file="./data/cod_6k5.json"):
    """
    ...
    def split_item(items: List[Any], test_size: float = 0.1, train_fold: int = 5, seed: int = 42) -> Tuple[List[Tuple[List[int], List[int]]], List[Any]]:
        """
    ...
    trains, test = split_item(dataset_raw, test_size=0.1, train_fold=len(all_gpu_ids), seed=42)
    ds_train = Dataset.from_list(trains[gpu_index])
    ds_test = Dataset.from_list(test)
    logger.debug(f"GPU {gpu_id}: Training on {len(ds_train)} samples, testing on {len(ds_test)} samples")
    ...
    trainer = SFTTrainer(
        ...
        dataset_num_proc=2,
        packing=False,
        callbacks=[WeightSyncCallback(gpu_id, all_gpu_ids, sync_interval=WEIGHT_SYNC_INTERVAL)],
