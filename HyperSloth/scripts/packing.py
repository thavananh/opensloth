...
def pack_by_indices(
    lengths: List[int], max_seq_len: int, shuffle_items: bool = True
) -> List[List[int]]:
    """
    ...
    if ignored_count > 0:
        ...
        logger.warning(
            f"Items ignored (too long): {ignored_count} ({skip_percentage:.2f}%)"
        )
    else:
    ...
    if batches:
        ...
        try:
            ...
            num_bins = min(
                30, len(set(batch_lengths)) // 2 if len(set(batch_lengths)) > 1 else 1
            )
...
def create_packed_items(
    ...
    for batch_indices in packed_indices:
        ...
        for idx in batch_indices:
            ...
            if "messages" in item and isinstance(item.get("messages"), list):
                messages_list = item["messages"]
            elif "conversations" in item and isinstance(
        ...
        packed_items.append({"messages": all_messages, "total_len": total_len})
...
def main():
    ...
    lengths = [
        get_len(item["messages"]) for item in tqdm(items, desc="Calculating lengths")
    ]
