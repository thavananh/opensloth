...
def setup_nccl_for_hypersloth(gpu: int, gpus: list) -> None:
    ...
    logger.info(f'[GPU={gpu}] NCCL environment variables set:')
    logger.info(f'[GPU={gpu}] RANK={os.environ["RANK"]}')
    logger.info(f'[GPU={gpu}] WORLD_SIZE={os.environ["WORLD_SIZE"]}')
    logger.info(f'[GPU={gpu}] MASTER_ADDR={os.environ["MASTER_ADDR"]}')
    logger.info(f'[GPU={gpu}] MASTER_PORT={os.environ["MASTER_PORT"]}')
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.info(
                    f'[GPU={gpu}] Retrying NCCL init in {retry_delay}s...'
                )
